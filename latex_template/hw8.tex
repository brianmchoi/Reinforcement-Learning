...
\newcommand{\courseName}{10-601 Introduction to Machine Learning (Fall 2020)}
\newcommand{\hwName}{Homework 8: Reinforcement Learning}
\newcommand{\dueDate}{Thursday, Dec 03, 2020 11:59 PM}
...
\begin{questions}
% \subsection{Value Iteration [6 Points]}
\label{sec:warmup}
...
\part[3] Compute the state values after each round of synchronous value iteration updates on the map of the maze before convergence. For example, after the first round, the values should look like this:
...
\begin{tcolorbox}[fit,height=3.5cm, width=\linewidth, blank, borderline={1pt}{-2pt},nobeforeafter]
    % YOUR ANSWER 
    % Fill in the tables by replacing the ??s with the appropriately calculated likelihood
...
\part[2] Which of the following changes will result in the same optimal policy as the settings above?
...
{\Large\question{Q-learning [8 Points]}}
...
\begin{parts}
\part[1] In the first episode, the agent takes action 7, receives +1 reward, and the episode terminates. Derive the updated Q-value after this episode. Remember that given the sampled experience $(s, a, r, s')$ of (state, action, reward, next state), the update of the Q value is:
...
\begin{your_solution}[title=Q(1\text{, }7),height=2cm,width=3cm]
    % YOUR ANSWER 
\end{your_solution}
...
\part[1] In the second episode, the agent takes action 8, receives a reward of -1, and the episode terminates. Derive the updated Q-value based on this episode.  \textbf{Please round to three decimal places}.

\begin{your_solution}[title=Q(1\text{, }8),height=2cm,width=3cm]
    % YOUR ANSWER 
\end{your_solution}
\begin{your_solution}[title=Work,height=4cm,width=12cm]
    % YOUR ANSWER 
...
\part[2] In the third episode, the agent takes action 3, receives a reward of 0, and arrives at State 2 (\ref{table:state2}). It then takes action 8, receives a reward of 0, and the episode terminates. Derive the updated Q-values after each of the two experiences in this episode. Suppose we update the corresponding Q-value right after every single step.
 \textbf{Please round to three decimal places}.

\begin{your_solution}[title=Q(1\text{, }3),height=2cm,width=3cm]
    % YOUR ANSWER 
\end{your_solution}
\begin{your_solution}[title=Q(2\text{, }8),height=2cm,width=3cm]
    % YOUR ANSWER 
\end{your_solution}
...
\part[2] If we run the three episodes in cycle forever, what will be the final values of the four Q-values.  \textbf{Please round to three decimal places}.
...
{\Large \question{Function Approximation [8 Points]}}
...
{\Large \question{Empirical questions [10 Points]}}
...
\part[2] We see that Figure~\ref{fig:value_b} seems to look like a plane. Can the value function depicted in this plot ever be nonlinear? If so, describe a potential shape. If not explain why. (\textbf{Hint:} How do we calculate the value of a state given the Q-values?)
